...
public enum CameraViewFinder {
    ...
    public static void drawCameraViewFinder(DrawContext context, TextRenderer textRenderer) {
        ...
        if (!Camerapture.CONFIG_MANAGER.getConfig().client.simpleCameraHud) {
            context.drawText(textRenderer, Text.translatable("text.camerapture.date", SDF_DATE.format(new Date())), textX, textY - fh, Colors.WHITE, false);
        }
        ...
        if (!CameraItem.canTakePicture(player)) {
            if (System.currentTimeMillis() % 1000 < 500) {
                ...
                int x = width / 2 - w / 2;
                int y = height / 2 + 32;
                context.drawText(textRenderer, Text.translatable("text.camerapture.no_paper"), x, y, Colors.RED, false);
            }
            ...
            int x = width - 25 - w;
            int y = height - 25 - fh;
            context.drawText(textRenderer, text, x, y, Colors.WHITE, false);
        }
    }
    ...
    private static void drawViewFinder(DrawContext context, int x1, int y1, int x2, int y2, int thickness, int length) {
        context.fill(x1, y1, x1 + length, y1 + thickness, Colors.WHITE);
        context.fill(x1, y1, x1 + thickness, y1 + length, Colors.WHITE);

        context.fill(x2 - length, y1, x2, y1 + thickness, Colors.WHITE);
        context.fill(x2 - thickness, y1, x2, y1 + length, Colors.WHITE);

        context.fill(x1, y2 - thickness, x1 + length, y2, Colors.WHITE);
        context.fill(x1, y2 - length, x1 + thickness, y2, Colors.WHITE);

        context.fill(x2 - length, y2 - thickness, x2, y2, Colors.WHITE);
        context.fill(x2 - thickness, y2 - length, x2, y2, Colors.WHITE);
    }
    ...
    private static void drawZoomBar(DrawContext context, TextRenderer textRenderer, int x, int y, int height) {
        ...
        context.fill(x - 10, ty - 1, x, ty + 1, Colors.WHITE);
        ...
        context.drawText(textRenderer, zoomLevel, x - 12 - textWidth, ty - 4, Colors.WHITE, false);
    }
}
